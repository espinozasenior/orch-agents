...
- Must preserve Promise.allSettled in `task-tool-strategy.ts run()` so partial results survive.
...
src/shared/event-types.ts      — Register 5 new event types in DomainEventType/DomainEventMap
src/execution/task-executor.ts — Extend TaskExecutionResult with tokenUsage?, add 'cancelled' status
...
src/index.ts                   — Add ENABLE_STREAMING_EXECUTOR env var to select executor
src/execution/work-tracker.ts  — Add optional agentTracker dep + getAgentsByPlan(planId) delegation
```

### WorkTracker ↔ AgentTracker Linkage

`WorkTracker` gains an optional `agentTracker?: AgentTracker` constructor dependency. When present, it exposes a `getAgentsByPlan(planId)` method that delegates to `agentTracker.getAgentsByPlan(planId)`. This keeps WorkTracker's core state machine unchanged while enabling drill-down queries. The AgentTracker is the source of truth for per-agent state; WorkTracker is a facade for callers who need both plan-level and agent-level views.

### Executor Selection Mechanism

A new environment variable `ENABLE_STREAMING_EXECUTOR=true` selects `StreamingTaskExecutor` over `createClaudeTaskExecutor` in `index.ts`. When both `ENABLE_TASK_AGENTS` and `ENABLE_STREAMING_EXECUTOR` are set, the streaming executor is used (it supersedes the basic executor). Default behavior (no env var) uses the existing `createClaudeTaskExecutor` for backward compatibility.
...
| Token parsing patterns speculative | MEDIUM | Claude CLI stderr format undocumented; tryParseTokens is best-effort with graceful fallback to tier estimates |
...
### Step 5: types.ts + event-types.ts + task-executor.ts updates

- Add AgentExecState, TokenUsage types to `src/types.ts`
- Register `AgentSpawned`, `AgentChunk`, `AgentCompleted`, `AgentFailed`, `AgentCancelled` in `src/shared/event-types.ts` DomainEventType and DomainEventMap
- Extend `TaskExecutionResult.status` to `'completed' | 'failed' | 'cancelled'`
- Add optional `tokenUsage?: { input: number; output: number }` to `TaskExecutionResult`

### Step 6: task-tool-strategy.ts + phase-runner.ts + tests (integration)

- Test: `task-tool-strategy.ts run()` with StreamingTaskExecutor preserves Promise.allSettled behavior
- Test: cancelled agent produces 'cancelled' not 'failed' in strategy results
- Note: The Promise.allSettled logic lives in `src/execution/strategies/task-tool-strategy.ts` `run()` method, NOT in `phase-runner.ts` `runTaskTool` (which does not exist)
...
| `src/shared/event-types.ts` | MODIFIED |

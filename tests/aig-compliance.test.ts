...
describe('AIG Compliance', () => {
  ...
  describe('Stop command detection (Linear)', () => {
    let linearServer: FastifyInstance;
    let linearBus: EventBus;
    let linearBuffer: EventBuffer;

    beforeEach(async () => {
      setWorkflowConfig(makeTestWorkflowConfig());
      setLinearBotUserId('');
      ...
      const logger = createLogger({ level: 'fatal' });
      linearBus = createEventBus();
      linearBuffer = createEventBuffer({ cleanupIntervalMs: 60_000 });

      linearServer = Fastify({ logger: false });
      await linearServer.register(linearWebhookHandler, {
        config,
        logger,
        eventBus: linearBus,
        eventBuffer: linearBuffer,
      } as LinearWebhookHandlerDeps);
      await linearServer.ready();
    });

    afterEach(async () => {
      resetWorkflowConfig();
      linearBuffer.dispose();
      linearBus.removeAllListeners();
      await linearServer.close();
    });

    it('should detect stop command in Linear comment', async () => {
      const cancelEvents: WorkCancelledEvent[] = [];
      linearBus.subscribe('WorkCancelled', (event) => {
        cancelEvents.push(event);
      });
      ...
      const response = await linearServer.inject({
        method: 'POST',
        url: '/webhooks/linear',
        headers: {
          'content-type': 'application/json',
          'linear-signature': computeLinearSignature(body, LINEAR_SECRET),
          'linear-delivery': 'linear-stop-1',
        },
        payload: body,
      });
      ...
      assert.equal(JSON.parse(response.body).status, 'cancelling');
      assert.equal(cancelEvents.length, 1);
      assert.ok(cancelEvents[0].payload.workItemId.includes('issue-42'));
    });

    it('should NOT trigger cancellation for non-stop Linear comments', async () => {
      const cancelEvents: WorkCancelledEvent[] = [];
      linearBus.subscribe('WorkCancelled', (event) => {
        cancelEvents.push(event);
      });
      ...
      const response = await linearServer.inject({
        ...
        headers: {
          'content-type': 'application/json',
          'linear-signature': computeLinearSignature(body, LINEAR_SECRET),
          'linear-delivery': 'linear-nostop-1',
        },
        payload: body,
      });

      assert.equal(response.statusCode, 202);
      assert.equal(JSON.parse(response.body).status, 'skipped');
      assert.equal(cancelEvents.length, 0, 'Should not publish cancellation for non-stop comments');
    });
  });
